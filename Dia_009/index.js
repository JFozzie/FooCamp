//crea el boton de Guardar lista
//const btnSaveLocal = document.getElementById("save-list");
const btnSaveLocal = document.createElement("button");
btnSaveLocal.innerText = "Guardar Lista";
btnSaveLocal.id = "save-list";
document.body.appendChild(btnSaveLocal);


const enableDisableBtn = () => {
  if ($itemsListContainer.children.length == 0) {
...
const saveListToLocal = () => {
  ...
  let data = [...document.querySelectorAll(`.${itemsListClass} li`)].map(element => element.innerText.slice(0, (element.innerText.length - 2)));
  let dCheck = [...document.querySelectorAll(`.${itemsListClass} li`)].map(element => element.innerHTML);
  localStorage.setItem("lista", JSON.stringify(data));
  console.log(data);
  console.log("Se salvo la lista");
}

const loadListFromLocal = () => {
  let readLista = JSON.parse(localStorage.getItem("lista"));
  console.log(readLista);
  readLista.forEach(item => createListElement(item));
}
...
const initListeners = () => {
  ...
  btnSaveLocal.addEventListener("click", saveListToLocal)
